...
void MainWindow::updateMenus()
{
	...
	exportMeshAsAct->setEnabled(activeDoc);
...
bool MainWindow::loadMesh(const QString& fileName, MeshIOInterface *pCurrentIOPlugin, MeshModel* mm, int& mask,RichParameterSet* prePar, const Matrix44m &mtr, bool isareload, MLRenderingData* rendOpt)
{
	...
	computeRenderingDataOnLoading(mm,isareload, rendOpt); //加载计算渲染的数据
	updateLayerDialog();
...
bool MainWindow::importMeshWithLayerManagement(QString fileName)
{
	...
	globrendtoolbar->setEnabled(false); //禁止使用工具栏
    bool res = importMesh(fileName,false);
	globrendtoolbar->setEnabled(true); //工具栏启动
	if (layerDialog != NULL)
...
bool MainWindow::importMesh(QString fileName,bool isareload)
{
    ...
    foreach(fileName,fileNameList)
    {
        ...
        pCurrentIOPlugin->initPreOpenParameter(extension, fileName,prePar); //当前打开的文件的初始化
        if(!prePar.isEmpty())
        {
        ...
        {
            meshDoc()->delMesh(mm);
			GLA()->Logf(0, "Warning: Mesh %s has not been opened", qUtf8Printable(fileName));//控制台输出打开mesh失败
        }
    }// end foreach file of the input list
    GLA()->Logf(0,"All files opened in %i msec",allFileTime.elapsed());//控制台输出打开mesh的时间
...
bool MainWindow::convertMesh(QString fileName, bool isareload)
{
	//控制台记录，记录打开文件是否成功，若成功输出时间
	if (!GLA())
	{
	...
	QStringList fileNameList;
	if (fileName.isEmpty())
		fileNameList = QFileDialog::getOpenFileNames(this, tr("Convert Mesh"), lastUsedDirectory.path(), PM.inpFilters.join(";;"));
	else
		fileNameList.push_back(fileName);
...
//
		...
		if (!fileName.isEmpty())
		{
			//save path away so we can use it again
			QString path = fileName;
			path.truncate(path.lastIndexOf("/"));//得到文件名
			lastUsedDirectory.setPath(path);//得到路径

			QString extension = fileName;
			extension.remove(0, fileName.lastIndexOf('.') + 1);//得到后缀名
			...
			qb->show();
			QTime tt; tt.start();
			ret = pCurrentIOPlugin->save(extension, fileName, *mm, mask, savePar, QCallBack, this);
			qb->reset();
			if (ret)
			{
...
bool MainWindow::saveAs(QString fileName,const bool isareload)
{
    return convertMesh(fileName, isareload);
}
