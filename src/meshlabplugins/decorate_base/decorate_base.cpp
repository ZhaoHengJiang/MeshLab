...
QString DecorateBasePlugin::decorationName(FilterIDType filter) const
{
    switch(filter)
    {
    case DP_SHOW_NORMALS:           return QString("Show Normal");//展示法线
    case DP_SHOW_CURVATURE:         return QString("Show Curvature");//展示曲率
    case DP_SHOW_BOX_CORNERS:       return QString("Show Box Corners");//展示方角
    case DP_SHOW_AXIS:              return QString("Show Axis");//展示坐标轴
    case DP_SHOW_LABEL:             return QString("Show Labels");//展示标签
    case DP_SHOW_CAMERA:            return QString("Show Camera");//展示
    case DP_SHOW_TEXPARAM:          return QString("Show UV Tex Param");//展示UV Tex Param
    case DP_SHOW_QUALITY_HISTOGRAM: return QString("Show Quality Histogram");//展示直方图
    case DP_SHOW_QUALITY_CONTOUR:   return QString("Show Quality Contour");//展示轮廓
    case DP_SHOW_SELECTED_MESH:     return QString("Show Current Mesh");//展示网格
    default: assert(0);
    }
...
void DecorateBasePlugin::decorateMesh(QAction *a, MeshModel &m, RichParameterSet *rm, GLArea *gla, QPainter *painter,GLLogStream &_log)
{
    ...
    case DP_SHOW_NORMALS:
        {
            glPushAttrib(GL_ENABLE_BIT ); //将GL_ENABLE_BIT压入属性栈
