...
class DecorateBasePlugin : public QObject, public MeshDecorateInterface //基础的绘制插件，如绘制坐标轴、顶点、边角、网格、特征柱状图等
{
  Q_OBJECT
  ...
  enum {
	DP_SHOW_AXIS,                    //绘制出坐标轴
	DP_SHOW_BOX_CORNERS,             //绘制出箱角 字面翻译
	DP_SHOW_SELECTED_MESH,           //绘制出选中的网格
	DP_SHOW_CAMERA,                  //绘制出影像
	DP_SHOW_NORMALS,                 //绘制法线
	DP_SHOW_TEXPARAM,                //绘制出纹理过滤参数 不确定
	DP_SHOW_QUALITY_HISTOGRAM,       //绘制出直方图
	DP_SHOW_QUALITY_CONTOUR,         //绘制出轮廓
	DP_SHOW_CURVATURE,               //绘制出曲率    
	DP_SHOW_LABEL,                   //绘制标签
  };
...
private:
  ...
  void drawHistogram(QGLWidget *gla, CHist &ch);//构建直方图

  vcg::Color4b textColor;
public:

  DecorateBasePlugin() //构造函数
  {
	  typeList <<
		DP_SHOW_AXIS <<
		DP_SHOW_BOX_CORNERS <<
